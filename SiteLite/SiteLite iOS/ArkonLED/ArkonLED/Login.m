...
- (IBAction)loginBtnClicked:(id)sender
{
    ...
    NSString *URL = [NSString stringWithFormat:@"%@/iOS/AccountInfo/login.php", [UserInfoGlobal serverURL]];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse * response, NSData * data,NSError * error)
     {
         if(!error)
         {
             ...
             if([[dicServerMessage objectForKey:@"success"] isEqualToNumber:@1])
             {
                 ...
                 [UserInfoGlobal setUserID:[NSString stringWithFormat:@"%ld", [[dicServerMessage objectForKey:@"userID"] longValue]]];
                 [UserInfoGlobal setUserType:[NSString stringWithFormat:@"%ld", [[dicServerMessage objectForKey:@"userType"] longValue]]];
